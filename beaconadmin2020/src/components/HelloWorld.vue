...
<v-container grid-list-md text-xs-center pt-2  pb-4 style="background-color:#80DEEA">
  ...
  <v-card>
    ...
    <v-flex v-show="LoginStatus" md12 v-for="(beaconInformation,n,number) in getContent " :key="number" class="text-md-left" pt-2>
      	<v-layout row wrap >
...
<!-- set -->
							...
							<v-flex md8 >
								...
								<v-flex md12 >
									<v-layout row wrap>
										...
										<v-flex md1 pa-0><h5>:</h5></v-flex>
										<v-flex md8 pa-0  ><h5>{{n}}</h5></v-flex>
									</v-layout>
								</v-flex>
								<v-flex md12  >
									<v-layout row wrap>
					...
					<v-flex  md2 pt-2 pr-2 style="border-left-style: outset;" >
						...
						<v-flex offset-md1 md10 style="">
							...
							block
							color="red darken-4" 
							@click.native="comfirmDeleteDialog(n, beaconInformation.title)" 
							>
